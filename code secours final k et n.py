...
while True:
  while pin8.read_digital() == True:
    while not getUltrasonicData(pin1, pin1, 'distance') < 10:
      codo_move('right', 600)
    if getUltrasonicData(pin1, pin1, 'distance') < 10:
      while getUltrasonicData(pin1, pin1, 'distance') < 10:
        codo_move('forward', 1023)
  if pin8.read_digital() != True:
    codo_move('backward', 900)
    utime.sleep(1.5)
